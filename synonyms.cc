...
#endif
	//printf("substitute_synonyms_cmdline 2\n");for (int jj=0;jj<nword;jj++)printf("\t'%s'\n",_Words2[jj]);
...
#if 0 // 2.060
		while (!isspace(sname[syn_len])) { // isspace seems superfluous
			if (end_of_synonym(sname[syn_len], inmath, need_brace))
#else
