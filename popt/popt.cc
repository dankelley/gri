...
poptContext poptGetContext(char * name ,int argc, char ** argv, 
	...
	con->leftovers = (char**)malloc(sizeof(char *) * (argc + 1));
	con->numLeftovers = 0;
	con->nextLeftover = 0;
	con->restLeftover = 0;
	con->options = options;
...
int poptAddAlias(poptContext con, struct poptAlias newAlias, int flags) {
	...
	*alias = newAlias;
	if (alias->longName)
		alias->longName = strcpy((char*)malloc(strlen(alias->longName) + 1), 
					 alias->longName);
	else
		alias->longName = NULL;
...
int poptParseArgvString(char * s, int * argcPtr, char *** argvPtr) {
	char * buf = strcpy((char*)alloca(strlen(s) + 1), s);
	char * bufStart = buf;
	char * src, * dst;
	char quote = '\0';
	int argvAlloced = 5;
	char ** argv = (char**)malloc(sizeof(*argv) * argvAlloced);
	char ** argv2;
	int argc = 0;
	int i;
	...
	dst = (char*)malloc(argc * sizeof(*argv) + (buf - bufStart));
	/*argv2 = (void *) dst;*/
	argv2 = (char **) dst;
...
int poptReadConfigFile(poptContext con, char * fn) {
	...
	lseek(fd, 0, 0);
  	...
  	file = (char *)mmap(NULL, fileLength, PROT_READ, MAP_PRIVATE, fd, 0);
	if (file == (void *) -1) {
		rc = errno;
		close(fd);
		errno = rc;
