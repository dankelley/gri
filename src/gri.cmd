#         gri - scientific graphic program (version 2.12.25)
#         Copyright 2016 by Dan E. Kelley; GPLv2+ licensing.
#
...
numerical model might have a trace of points representing a river).
   ...
   With the `size' options `.width.' and `.height.' specified, they
set the number of rectanglular patches in the image.  Interpolation is
used.

   With the `box' options specified, they set the bounding box for the
image.  Otherwise, by default, the image spans the same bounding box
as the grid as set by `set x grid' and `set y grid'.  Again,
interpolation is used in this form.
